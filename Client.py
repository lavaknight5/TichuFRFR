...
playSelected = pygame.transform.scale(pygame.image.load("Assets/PlaySelected.png"), (WIDTH/10, HEIGHT/15)).convert_alpha()
playGrey = pygame.transform.scale(pygame.image.load("Assets/PlayGrey.png"), (WIDTH/10, HEIGHT/15)).convert_alpha()
play = pygame.transform.scale(pygame.image.load("Assets/Play.png"), (WIDTH/10, HEIGHT/15)).convert_alpha()
...
cardrect = {}
#playPos = [WIDTH - (3 * cardWidth * (2 / 3)), HEIGHT - cardHeight + 20]

def drawGame(hand, playImg, length, player):
    WIN.blit(BG, (0, 0))
    i = 0
    space = (len(hand)+1) * (cardWidth * (2 / 3))
    playPos = ((15 * (cardWidth * (2 / 3))) + cardWidth/2, HEIGHT - cardHeight - 20*bols)
    WIN.blit(playImg, (playPos[0], playPos[1]))
    for card in hand:
    ...
    if player == 3:
        ...
        right = 0
        opposite = 1
    spaceOp = (length[opposite] + 1) * (cardWidth * (2 / 3))
...
def inGame():
    ...
    while run:
                ...
                if phase == "Playing" or selMode:
                    for i in range(len(hand)):
                        if playing and cardrect[i].collidepoint(x_mouse, y_mouse) and not hand[i].selected:
                            ...
                            try:
                                if phase == "Playing" and (hand[13].selected or hand[12].selected or hand[11].selected):
                                    if bols == 5:
                                        bols = 8
                                    elif bols == 4:
                                        bols = 6
        ...
        if phase == "Playing" and playing:
            drawGame(hand, play, PlayerHands, player)
        elif phase == "Giving" and playing:
            drawGiving(hand, x_mouse, y_mouse)
        else:
            drawWaitingScreen()
    pygame.quit()

def drawGiving(hand, x, y):
    WIN.blit(BG, (0, 0))
    i = 0
    space = (len(hand) + 1) * (cardWidth * (2 / 3))
...
def menu():
    ...
    #game = g(1)
    #deck = game.getDeck(cardWidth, cardHeight, cardImage)
